        ...
        for idx in pbar:
            ...
            if record:
                if idx % 10 == 0:
                    ...
                    # 直接保存到指定目录，不再创建额外的progress子目录
                    file_path = os.path.join(save_root, f"x_{str(idx).zfill(4)}.png")
                    plt.imsave(file_path, clear_color(img[0].unsqueeze(0)))
                    file_path = os.path.join(save_root, f"y_{str(idx).zfill(4)}.png")
                    plt.imsave(file_path, clear_color(y[0].unsqueeze(0)))

        return torch.unsqueeze(img[0], 0), []  # 返回最终结果和空的中间结果列表
